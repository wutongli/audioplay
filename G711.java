package com.telyes.ivmcc.tpu.gb28181.codec;
...
public class G711 {
    ...
    public static int encode( byte[] in,byte[] out,int outOffset){
        int j = 0;
        int count = in.length / 2;
        int end = outOffset+count;
        short sample = 0;

        for ( int i = outOffset; i < end; i++ )
        {
     ...
     * @return
     */
    public static byte[] decode( byte[] in,int inOffset,int len){
        int j = 0;
        byte[] res = new byte[len*2];
        decode(in,inOffset,len,res,0);
        return res;
    }
     ...
     * @return
     */
    public static int decode( byte[] in,int inOffset,int len,byte[] out,int outOffset){
        int j = outOffset;

        int end = inOffset + len;
        for ( int i = inOffset; i < end; i++ )
        {
